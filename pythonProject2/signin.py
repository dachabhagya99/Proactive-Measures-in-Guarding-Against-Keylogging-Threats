...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        if row==None:
            messagebox.showerror('Erroor','Invalid Username or Password')
        else:
...
T_logo=PhotoImage(file='twitter.png')
T_Label=Label(login_window,image=T_logo,bd=0,bg='white')
T_Label.place(x=690,y=440)

G_logo=PhotoImage(file='google.png')
GLabel=Label(login_window,image=G_logo,bd=0,bg='white')
GLabel.place(x=740,y=440)

orLable=Label(login_window,text='Do Not Have Account?',font=('Open sans',9,'bold'),fg='firebrick1',
              bd=0,bg='white')
orLable.place(x=590,y=500)
